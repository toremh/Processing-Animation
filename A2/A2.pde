...
void nightCycle() {
  if (nightTimer >=0 && nightTimer < 300) {
    int i1 = int(nightTimer)/30;
    int c1 = int(skycolors[i1][0]);
    int c2 = int (skycolors[i1+1][0]);
    int c3 = int(skycolors[i1][1]);
    int c4 = int (skycolors[i1+1][1]);
    skyUp = lerpColor(c1, c2, nightTimer%30/30);
    skyDown = lerpColor(c3, c4, nightTimer%30/30);
  }
    if (nightTimer >= 350 && nightTimer <650) {
    float thisTimer = 650 - nightTimer;
    int i1 = int(thisTimer)/30;
    int i2 = i1;
    if(i2>0){i2--;}
    int c1 = int(skycolors[i1][0]);
    int c2 = int (skycolors[i2][0]);
    int c3 = int(skycolors[i1][1]);
    int c4 = int (skycolors[i2][1]);
    skyUp = lerpColor(c1, c2, 1-thisTimer%30/30);
    skyDown = lerpColor(c3, c4, 1-thisTimer%30/30);
  }
  //if (nightTimer >= 300 && nightTimer <600) {
  //  int i1 = 10-int(nightTimer)/30;
  //  int i2 = i1;
  //  if(i2>0){i2--;}
  //  int c1 = int(skycolors[i1][0]);
  //  int c2 = int (skycolors[i2][0]);
...
void drawSky() {
  for (int i = 0; i <= 750; i++) {
    float inter = map(i, 0, 750, 0, 1);
    color cTemp = lerpColor(skyUp, skyDown, inter);
    stroke(cTemp);
    line(0, i, 1000, i);
  }
}
